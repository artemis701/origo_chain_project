...
## default port prefixes for origod
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/origod keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/origod init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/origod add-genesis-account \
    "$("$PWD"/build/origod keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000pose,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/origod gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/origod collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/origod validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/origod start --pruning=nothing --rpc.unsafe \
