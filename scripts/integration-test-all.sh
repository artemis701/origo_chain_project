...
## default port prefixes for exad
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/exad keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/exad init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/exad add-genesis-account \
    "$("$PWD"/build/exad keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000pose,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/exad gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/exad collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/exad validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/exad start --pruning=nothing --rpc.unsafe \
