...
// Makes system call to current node container environment with exad cli command to get current block height
func (m *Manager) nodeHeight(ctx context.Context) (int, error) {
	exec, err := m.CreateExec([]string{"exad", "q", "block"}, m.ContainerID())
	if err != nil {
