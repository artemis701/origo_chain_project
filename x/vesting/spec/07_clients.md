...
Find below a list of `origod` commands added with the `x/vesting` module. You can obtain the full list by using the `origod -h` command.
...
```go
origod add-genesis-account ADDRESS_OR_KEY_NAME COIN... [flags]
```
...
```go
origod query vesting balances ADDRESS [flags]
```
...
```go
origod tx vesting create-clawback-vesting-account TO_ADDRESS [flags]
```
...
```go
origod tx vesting clawback ADDRESS [flags]
```
...
```go
origod tx vesting update-vesting-funder VESTING_ACCOUNT_ADDRESS NEW_FUNDER_ADDRESS --from=FUNDER_ADDRESS [flags]
```
