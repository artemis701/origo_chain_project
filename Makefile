...
build-docker:
	...
	mkdir -p ./build/
	$(DOCKER) cp evmos:/usr/bin/origod ./build/
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(EVMOS_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/evmos:Z evmos/node "./origod testnet init-files --v 4 -o /evmos --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\origod:/evmos\Z evmos/node "./origod tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\build\node1\origod:/evmos\Z evmos/node "./origod tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\build\node2\origod:/evmos\Z evmos/node "./origod tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\build\node3\origod:/evmos\Z evmos/node "./origod tendermint unsafe-reset-all --home=/evmos"
else
	@docker run --rm -v $(CURDIR)/build/node0/origod:/evmos:Z evmos/node "./origod tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/build/node1/origod:/evmos:Z evmos/node "./origod tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/build/node2/origod:/evmos:Z evmos/node "./origod tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/build/node3/origod:/evmos:Z evmos/node "./origod tendermint unsafe-reset-all --home=/evmos"
endif
