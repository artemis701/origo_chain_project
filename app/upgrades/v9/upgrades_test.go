...
func (suite *UpgradeTestSuite) TestReturnFundsFromCommunityPool() {
	...
	coins := sdk.NewCoins(sdk.NewCoin("cmu", res))
	...
	suite.Require().Equal(balanceBefore.AmountOf("cmu"), sdk.NewDecFromInt(res))
	...
	for i := range v9.Accounts {
		...
		balance := suite.app.BankKeeper.GetBalance(suite.ctx, addr, "cmu")
