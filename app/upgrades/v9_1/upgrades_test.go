	...
	testCases := []struct {
	...
	}{
		{
			...
			func() {
				...
				coins := sdk.NewCoins(sdk.NewCoin("exa", res))
				...
				suite.Require().Equal(balanceBefore.AmountOf("exa"), sdk.NewDecFromInt(res))
			},
			true,
		},
		{
		...
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			if tc.expectedSuccess {
				for i := range v9.Accounts {
					...
					balance := suite.app.BankKeeper.GetBalance(suite.ctx, addr, "exa")
