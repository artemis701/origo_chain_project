...
exad tx revenue register $CONTRACT $NONCE $WITHDRAWER \
...
```bash
# Check revenues
exad q revenue contract $CONTRACT
```
...
exad tx revenue update $CONTRACT $WITHDRAWER \
...
exad tx revenue cancel $CONTRACT \
--gas=700000 --gas-prices=10000pose \
--from=mm
```
