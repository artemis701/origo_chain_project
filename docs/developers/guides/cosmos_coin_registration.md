...
To [submit the proposal](../../users/governance/submitting.md) to testnet through the command line with [`exad`](../../validators/quickstart/binary.md), use the following command with `register-coin`:

```bash
exad tx gov submit-proposal register-coin <path/to/metadata.json> \
  --title=<title> \
...
```bash
exad tx gov submit-proposal register-coin --proposal=<path/to/proposal.json>
```
