...
To [submit the proposal](../../users/governance/submitting.md) to testnet through the command line with [`origod`](../../validators/quickstart/binary.md), use the following command with `register-coin`:

```bash
origod tx gov submit-proposal register-coin <path/to/metadata.json> \
  --title=<title> \
...
```bash
origod tx gov submit-proposal register-coin --proposal=<path/to/proposal.json>
```
