...
- Edit `addr` to point to your `origod` instance.
- Adjust `chain-id` to match your `.origod/config/config.toml` settings.
...
The last step is to retrieve the validator key that you will use in `origod`.
...
The KMS may complain that it cannot connect to `origod`. That is fine, we will fix it in the next section.
...
You need to enable KMS access by editing `.origod/config/config.toml`. In this file, modify `priv_validator_laddr` to create a listening address/port or a unix socket in `origod`.
...
Let's assume that you have set up your validator account and called it `kmsval`. You can tell origod the key that we've got in the previous section.

```bash
origod gentx --name kmsval --pubkey <pub_key>
```

Now start `origod`. You should see that the KMS connects and receives a signature request.
