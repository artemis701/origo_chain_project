...
Before upgrading the Evmos version. Stop your instance of `exad` using `Ctrl/Cmd+C`.
...
::: tip
If the software version does not match, then please check your `$PATH` to ensure the correct `exad` is running.
:::
...
```bash
rm $HOME/.exad/config/addrbook.json
