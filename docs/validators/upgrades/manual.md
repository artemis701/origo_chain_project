...
Before upgrading the Evmos version. Stop your instance of `origod` using `Ctrl/Cmd+C`.
...
::: tip
If the software version does not match, then please check your `$PATH` to ensure the correct `origod` is running.
:::
...
```bash
rm $HOME/.origod/config/addrbook.json
