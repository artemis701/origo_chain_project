...
```bash
exad init <your_custom_moniker> --chain-id evmos_9000-4
```
...
By default, the `init` command creates your `~/.exad` (i.e `$HOME`) directory with subfolders `config/` and `data/`.
...
Check the `genesis.json` file from the [`archive`](https://archive.evmos.dev/evmos_9000-4/genesis.json) and copy it over to the `config` directory: `~/.exad/config/genesis.json`. This is a genesis file with the chain-id and genesis accounts balances.

```bash
sudo apt install -y unzip wget
wget -P ~/.exad/config https://archive.evmos.dev/evmos_9000-4/genesis.json
```
...
Your node needs to know how to find [peers](https://docs.tendermint.com/v0.34/tendermint-core/using-tendermint.html#peers). You'll need to add healthy [seed nodes](https://docs.tendermint.com/v0.34/tendermint-core/using-tendermint.html#seed) to `$HOME/.exad/config/config.toml`. The [`testnets`](https://github.com/evmos/testnets) repo contains links to some seed nodes.

Edit the file located in `~/.exad/config/config.toml` and the `seeds` to the following:
...
sed -i.bak -e "s/^seeds =.*/seeds = \"$SEEDS\"/" ~/.exad/config/config.toml
```
...
We can set the [`persistent_peers`](https://docs.tendermint.com/v0.34/tendermint-core/using-tendermint.html#persistent-peer) field in `~/.exad/config/config.toml` to specify peers that your node will maintain persistent connections with. You can retrieve them from the list of
...
```bash
sed -i.bak -e "s/^persistent_peers *=.*/persistent_peers = \"$PEERS\"/" ~/.exad/config/config.toml
```
...
```bash
exad tx staking create-validator \
  --amount=1000000000000atevmos \
  --pubkey=$(exad tendermint show-validator) \
  --moniker="EvmosWhale" \
  --chain-id=<chain_id> \
...
```bash
rm $HOME/.exad/config/addrbook.json $HOME/.exad/config/genesis.json
exad tendermint unsafe-reset-all --home $HOME/.exad
```
