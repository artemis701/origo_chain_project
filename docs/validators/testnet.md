...
```bash
origod init <your_custom_moniker> --chain-id evmos_9000-4
```
...
By default, the `init` command creates your `~/.origod` (i.e `$HOME`) directory with subfolders `config/` and `data/`.
...
Check the `genesis.json` file from the [`archive`](https://archive.evmos.dev/evmos_9000-4/genesis.json) and copy it over to the `config` directory: `~/.origod/config/genesis.json`. This is a genesis file with the chain-id and genesis accounts balances.

```bash
sudo apt install -y unzip wget
wget -P ~/.origod/config https://archive.evmos.dev/evmos_9000-4/genesis.json
```
...
Your node needs to know how to find [peers](https://docs.tendermint.com/v0.34/tendermint-core/using-tendermint.html#peers). You'll need to add healthy [seed nodes](https://docs.tendermint.com/v0.34/tendermint-core/using-tendermint.html#seed) to `$HOME/.origod/config/config.toml`. The [`testnets`](https://github.com/evmos/testnets) repo contains links to some seed nodes.

Edit the file located in `~/.origod/config/config.toml` and the `seeds` to the following:
...
sed -i.bak -e "s/^seeds =.*/seeds = \"$SEEDS\"/" ~/.origod/config/config.toml
```
...
We can set the [`persistent_peers`](https://docs.tendermint.com/v0.34/tendermint-core/using-tendermint.html#persistent-peer) field in `~/.origod/config/config.toml` to specify peers that your node will maintain persistent connections with. You can retrieve them from the list of
...
```bash
sed -i.bak -e "s/^persistent_peers *=.*/persistent_peers = \"$PEERS\"/" ~/.origod/config/config.toml
```
...
```bash
origod tx staking create-validator \
  --amount=1000000000000atevmos \
  --pubkey=$(origod tendermint show-validator) \
  --moniker="EvmosWhale" \
  --chain-id=<chain_id> \
...
```bash
rm $HOME/.origod/config/addrbook.json $HOME/.origod/config/genesis.json
...
```bash
origod tendermint show-node-id
```
