...
```bash
$ origod keys add dev0-restored --recover
...
```bash
origod keys unsafe-export-eth-key dev0 > dev0.export
...
```bash
$ origod keys import dev0-imported ./dev0.export
Enter passphrase to decrypt your key:
```
...
```
$ origod keys unsafe-import-eth-key dev0-imported ./dev0.export
