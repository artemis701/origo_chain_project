...
```bash
$ exad keys add dev0-restored --recover
...
```bash
exad keys unsafe-export-eth-key dev0 > dev0.export
...
```bash
$ exad keys import dev0-imported ./dev0.export
Enter passphrase to decrypt your key:
```
...
```
$ exad keys unsafe-import-eth-key dev0-imported ./dev0.export
